...
samples=c('Se','Si','ST','Je','Ji','JT')
features<-c('Nucb2')
i=1
for (obj in c(scRNASe,scRNASi,scRNAST,scRNAJe,scRNAJi,scRNAJT)){
  sample=samples[i]
  
  pdf(file=sprintf("Nucb2_%s.pdf",sample),height = 6, width = 6)
  par(mfrow = c(2,1), xpd=TRUE)
  FeaturePlot(object = obj, features = features)
  print(FeaturePlot(object = obj, features = features))
  DotPlot(obj,group.by = 'cellType', features = unique(features),cluster.idents = T) + coord_flip()
...
####脂肪细胞提取
scRNASeAP=subset(x = scRNASe, subset = (cellType == "AP"))
scRNAJeAP=subset(x = scRNAJe, subset = (cellType == "AP"))
scRNASiAP=subset(x = scRNASi, subset = (cellType == "AP"))
scRNAJiAP=subset(x = scRNAJi, subset = (cellType == "AP"))
scRNASTAP=subset(x = scRNAST, subset = (cellType == "AP"))
scRNAJTAP=subset(x = scRNAJT, subset = (cellType == "AP"))
DefaultAssay(scRNA) <- "RNA"
scRNAAP=subset(x = scRNA, subset = (cellType == "AP"))

Idents(scRNAAP) <- "orig.ident"
SeJe.response <- FindMarkers(scRNAAP, 
                                     ident.1 = "Se", 
                                     ident.2 = "Je", 
                                     verbose = FALSE)
# head(SeJe.response, n = 20)

scRNASeJeAP=subset(x = scRNAAP, subset = (orig.ident == "Se"|orig.ident == "Je"))
features=c(rownames(head(SeJe.response,n=20)))
# pdf(file="SeJe-featureplot-diff.pdf",height = 25, width = 4)
# FeaturePlot(scRNASeJeAP,
...
#             reduction = "umap")
# dev.off()
pdf(file="SeJe-AP-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASeJeAP,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()

SiJi.response <- FindMarkers(scRNAAP, 
                                     ident.1 = "Si", 
                                     ident.2 = "Ji", 
                                     verbose = FALSE)
# head(SiJi.response, n = 20)                                     
scRNASiJiAP=subset(x = scRNAAP, subset = (orig.ident == "Si"|orig.ident == "Ji"))
features=c(rownames(head(SiJi.response,n=20)))
# pdf(file="SiJi-featureplot-diff.pdf",height = 25, width = 4)
# FeaturePlot(scRNASiJiAP,
#             features = c("Fkbp5","Cish","Pik3r1","Klb","Car3","Zbtb16","Cfd","Bank1","Nnat","Gtf2ird1","Malat1","Arhgap15","Ptprc","Acaca","Pnpla3"), 
#             split.by = "orig.ident", 
#             max.cutoff = 3, 
#             cols = c("grey", "red"), 
#             reduction = "umap")
# dev.off()
pdf(file="SiJi-AP-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASiJiAP,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()


STJT.response <- FindMarkers(scRNAAP, 
                                     ident.1 = "ST", 
                                     ident.2 = "JT", 
                                     verbose = FALSE)
features=c(rownames(head(STJT.response,n=20)))                                    
scRNASTJTAP=subset(x = scRNAAP, subset = (orig.ident == "ST"|orig.ident == "JT"))
# pdf(file="STJT-featureplot-diff.pdf",height = 25, width = 4)
# FeaturePlot(scRNASTJTAP,
#             features = c("A830018L16Rik","Nabp1","Fam126b","MyL1","Acsl3","Pid1","Dgkd","Ivnslabp","Dcaf6","Mpc2","Adcy10","Esrrg","Ppp2r5a","Hsd11b1","Agpat2"), 
#             split.by = "orig.ident", 
#             max.cutoff = 3, 
#             cols = c("grey", "red"), 
#             reduction = "umap")
# dev.off()
pdf(file="STJT-AP-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASTJTAP,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()
# -----------------------------------
scRNAFB=subset(x = scRNA, subset = (cellType == "FB"))
Idents(scRNAFB) <- "orig.ident"
SeJe.response <- FindMarkers(scRNAFB, 
                                     ident.1 = "Se", 
                                     ident.2 = "Je", 
                                     verbose = FALSE)
# head(SeJe.response, n = 20)
write.csv(file="SeJe.response.FB.csv",SeJe.response)
scRNASeJeFB=subset(x = scRNAFB, subset = (orig.ident == "Se"|orig.ident == "Je"))
features=c(rownames(head(SeJe.response,n=20)))
pdf(file="SeJe-FB-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASeJeFB,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()

SiJi.response <- FindMarkers(scRNAFB, 
                                     ident.1 = "Si", 
                                     ident.2 = "Ji", 
                                     verbose = FALSE)
# head(SiJi.response, n = 20)
write.csv(file="SiJi.response.FB.csv",SiJi.response)                                     
scRNASiJiFB=subset(x = scRNAFB, subset = (orig.ident == "Si"|orig.ident == "Ji"))
features=c(rownames(head(SiJi.response,n=20)))
pdf(file="SiJi-FB-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASiJiFB,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()


STJT.response <- FindMarkers(scRNAFB, 
                                     ident.1 = "ST", 
                                     ident.2 = "JT", 
                                     verbose = FALSE)
features=c(rownames(head(STJT.response,n=20)))
write.csv(file="STJT.response.FB.csv",STJT.response)                                      
scRNASTJTFB=subset(x = scRNAFB, subset = (orig.ident == "ST"|orig.ident == "JT"))
pdf(file="STJT-FB-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASTJTFB,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()
# -----------------------------------------
scRNAMP=subset(x = scRNA, subset = (cellType == "MP"))
Idents(scRNAMP) <- "orig.ident"
SeJe.response <- FindMarkers(scRNAMP, 
                                     ident.1 = "Se", 
                                     ident.2 = "Je", 
                                     verbose = FALSE)
# head(SeJe.response, n = 20)

scRNASeJeMP=subset(x = scRNAMP, subset = (orig.ident == "Se"|orig.ident == "Je"))
features=c(rownames(head(SeJe.response,n=20)))
pdf(file="SeJe-MP-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASeJeMP,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()

SiJi.response <- FindMarkers(scRNAMP, 
                                     ident.1 = "Si", 
                                     ident.2 = "Ji", 
                                     verbose = FALSE)
# head(SiJi.response, n = 20)                                     
scRNASiJiMP=subset(x = scRNAMP, subset = (orig.ident == "Si"|orig.ident == "Ji"))
features=c(rownames(head(SiJi.response,n=20)))
pdf(file="SiJi-MP-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASiJiMP,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()


STJT.response <- FindMarkers(scRNAMP, 
                                     ident.1 = "ST", 
                                     ident.2 = "JT", 
                                     verbose = FALSE)
features=c(rownames(head(STJT.response,n=20)))                                    
scRNASTJTMP=subset(x = scRNAMP, subset = (orig.ident == "ST"|orig.ident == "JT"))
pdf(file="STJT-MP-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASTJTMP,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()
# ---------------------------------------------
scRNAmyoFB=subset(x = scRNA, subset = (cellType == "myoFB"))
Idents(scRNAmyoFB) <- "orig.ident"
SeJe.response <- FindMarkers(scRNAmyoFB, 
                                     ident.1 = "Se", 
                                     ident.2 = "Je", 
                                     verbose = FALSE)
# head(SeJe.response, n = 20)
write.csv('SeJe.response-myoFB.response.csv',SeJe.response)
scRNASeJemyoFB=subset(x = scRNAmyoFB, subset = (orig.ident == "Se"|orig.ident == "Je"))
features=c(rownames(head(SeJe.response,n=20)))
pdf(file="SeJe-myoFB-diffdotplot.pdf",height = 6, width = 6)
DotPlot(scRNASeJemyoFB,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()

SiJi.response <- FindMarkers(scRNAmyoFB, 
                                     ident.1 = "Si", 
                                     ident.2 = "Ji", 
                                     verbose = FALSE)
# head(SiJi.response, n = 20)                                     
scRNASiJimyoFB=subset(x = scRNAmyoFB, subset = (orig.ident == "Si"|orig.ident == "Ji"))
features=c(rownames(head(SiJi.response,n=20)))
pdf(file="SiJi-myoFB-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASiJimyoFB,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()


STJT.response <- FindMarkers(scRNAmyoFB, 
                                     ident.1 = "ST", 
                                     ident.2 = "JT", 
                                     verbose = FALSE)
features=c(rownames(head(STJT.response,n=20)))                                    
scRNASTJTmyoFB=subset(x = scRNAmyoFB, subset = (orig.ident == "ST"|orig.ident == "JT"))
pdf(file="STJT-myoFB-diffdotplot.pdf",,height = 6, width = 6)
DotPlot(scRNASTJTmyoFB,group.by = 'orig.ident', features = unique(features),cluster.idents = T) + coord_flip()
dev.off()
...
scRNASe@meta.data$cellType=factor(scRNASe@meta.data$cellType,levels =c("AP","B","mDC","EC","FB","MP","Mono","muscle","NK","myoFB","SMC","T","Unknown"))
scRNASi@meta.data$cellType=factor(scRNASi@meta.data$cellType,levels =c("AP","B","mDC","EC","FB","MP","Mono","muscle","NK","myoFB","SMC","T","Unknown"))
scRNAST@meta.data$cellType=factor(scRNAST@meta.data$cellType,levels =c("AP","B","mDC","EC","FB","MP","Mono","muscle","NK","myoFB","SMC","T","Unknown"))
scRNAJe@meta.data$cellType=factor(scRNAJe@meta.data$cellType,levels =c("AP","B","mDC","EC","FB","MP","Mono","muscle","NK","myoFB","SMC","T","Unknown"))
scRNAJi@meta.data$cellType=factor(scRNAJi@meta.data$cellType,levels =c("AP","B","mDC","EC","FB","MP","Mono","muscle","NK","myoFB","SMC","T","Unknown"))
scRNAJT@meta.data$cellType=factor(scRNAJT@meta.data$cellType,levels =c("AP","B","mDC","EC","FB","MP","Mono","muscle","NK","myoFB","SMC","T","Unknown"))

i=1
for (obj in c(scRNASe,scRNASi,scRNAST,scRNAJe,scRNAJi,scRNAJT)){
  sample=samples[i]
  pdf(file=sprintf("Nucb2_%s_s.pdf",sample),height = 6, width = 6)
  par(mfrow = c(2,1), xpd=TRUE)
  # FeaturePlot(object = obj, features = features)
  # print(FeaturePlot(object = obj, features = features))
  DotPlot(obj,group.by = 'cellType', features = unique(features)) + coord_flip()+RotatedAxis()
  print(DotPlot(obj,group.by = 'cellType', features = unique(features)) + coord_flip()+RotatedAxis())
  dev.off()
  i=i+1
}
...
pdf(file="test2.pdf",height = 6, width = 9)
DotPlot(scRNASeJe,group.by = c('cellType'), features = unique(features),split.by = 'orig.ident') + coord_flip()+RotatedAxis()
dev.off()

scRNASeJe@meta.data$group <- paste('Nucb2', scRNASeJe@meta.data$orig.ident, sep = "_")

scRNASiJi@meta.data$group <- paste(data.features$id, splits, sep = "_")
scRNASTJT@meta.data$group <- paste(data.features$id, splits, sep = "_")

samples=c('SeJe','SiJi','STJT')
features<-c('Nucb2_Se','Nucb2_Je')
i=1
for (obj in c(scRNASeJe,scRNASiJi,scRNASTJT)){
  sample=samples[i]
  pdf(file=sprintf("Nucb2_%s.pdf",sample),height = 6, width = 9)
  ...
  DotPlot(obj,group.by = 'group', features = unique(features),cluster.idents = T) + coord_flip()
  print(DotPlot(obj,group.by = 'group', features = unique(features),cluster.idents = T) + coord_flip()+RotatedAxis())
  dev.off()
  i=i+1
}
...
p=p1+geom_point(data = as.data.frame(pos_gene1Se),
  ...
  geom_point(data = as.data.frame(pos_gene1Je),
             aes(x=UMAP_1,y=UMAP_2),
             shape = 16,size = 1.4, color="#ff69b4")
ggsave(filename = "SeJe-Dimplotgene.pdf", height = 7, width = 7, plot =p)
...
p=p1+geom_point(data = as.data.frame(pos_gene1Se),
  ...
  geom_point(data = as.data.frame(pos_gene1Je),
             aes(x=UMAP_1,y=UMAP_2),
             shape = 16,size = 1.4, color="#ff69b4")
ggsave(filename = "SiJi-Dimplotgene.pdf", height = 7, width = 7, plot =p)
...
pos_gene=scRNASTJT@reductions$umap@cell.embeddings

pos_gene1=pos_gene[scRNASTJT@assays$RNA@data[gene1,]>0,]

pos_gene1Se=pos_gene1[grep('ST',rownames(pos_gene1)),]
pos_gene1Je=pos_gene1[grep('JT',rownames(pos_gene1)),]
...
p=p1+geom_point(data = as.data.frame(pos_gene1Se),
  ...
  geom_point(data = as.data.frame(pos_gene1Je),
             aes(x=UMAP_1,y=UMAP_2),
             shape = 16,size = 1.4, color="#ff69b4")
ggsave(filename = "STJT-Dimplotgene.pdf", height = 7, width = 7, plot =p)
...
for (obj in c(scRNASeJe,scRNASiJi,scRNASTJT)){
  ...
  p_list <- list()
  i=1
  for (ct in c("AP","B","mDC","EC","FB","MP","Mono","muscle","NK","myoFB","SMC","T")){
    obj_ct=subset(x = obj, subset = (cellType == ct))
    p=VlnPlot(object = obj_ct, features = features,group.by='orig.ident')+ theme(text = element_text(size = 30))+theme(axis.text.x = element_text(face="bold", size=20),
           axis.text.y = element_text(face="bold",  size=20))
    p_list[[i]] <- p
    i=i+1
  }
  grid_plot_up <- plot_grid(plotlist =p_list , nrow = 3,ncol = 4)

  ggsave(
    plot = grid_plot_up,
    sprintf("%s.pdf",sample),
    height = 20,
    width = 20
  )
  n=n+1
...
library(VennDiagram)
l1=c("Fkbp5","Lcn8","Lcn5","Lcn9","Defb20","Spag11b","Gpx5","Adam7","Fabp4","Nnat","Pate13","Defb25","Pik3r1","Slc38a5","Defb48","Dnajc6","Gm1110","Rnf144b","Clu","Lcn2")
l2=c("Fkbp5","Cish","Pik3r1","Klb","Car3","Zbtb16","Cfd","Bank1","Nnat","Gtf2ird1","Malat1","Arhgap15","Ptprc","Acaca","Pnpla3","Acacb","Fabp4","Ghr","Gsn","Dock2")
l3=c("A830018L16Rik" ,"Nabp1", "Fam126b", "Myl1" ,"Acsl3","Pid1", "Dgkd", "Ivns1abp","Dcaf6", "Mpc2","Adcy10","Esrrg","Ppp2r5a","Hsd11b1","Agpat2", "Gpd2","Stk39","Ttn","Hsd17b12","Zc3h6")

venn_list <- list(e = l1, i = l2 ,T=l3)
p_venn <- ggvenn(venn_list,show_percentage = T,
                            stroke_color = "white",
                            fill_color = c("#BC3C29FF","#0072B5FF","#E18727FF","#FFDC91FF",
                                           "#EE4C97FF","#20854EFF","#7876B1FF","#6F99ADFF" ),
                            set_name_color =c("#E41A1C","#1E90FF","#6F99ADFF"))
...
p1=DimPlot(scRNASeJeAP,group.by = "orig.ident")
# 查看聚类信息
p2=DimPlot(scRNASeJeAP,group.by = "seurat_clusters",label=T)
combine<-CombinePlots(list(p1,p2))
ggsave(filename = "p_DimPlot_sample_seurat_clusters.pdf", plot = combine, height = 4, width = 7)

markers <- FindAllMarkers(scRNASeJeAP, only.pos = TRUE)
write.csv(filename = "submarkers.csv",markers)





pdf(file="subanno-Nucb2-clusterdot.pdf",height = 4, width = 4)
DotPlot(scRNASeJeAP,group.by = 'seurat_clusters', features = unique(c('Nucb2'))) + coord_flip()+RotatedAxis()
dev.off()

pdf(file="subanno-Nucb2-clustervln.pdf",height = 6, width = 9)
VlnPlot(scRNASeJeAP, features = c('Nucb2'), slot = "counts", log = TRUE)
dev.off()

pdf(file="subanno-Nucb2-clusterfeatureplot.pdf",height = 6, width = 9)
FeaturePlot(scRNASeJeAP, features = c('Nucb2'))
dev.off()
...
pdf(file="subanno-Nucb2-Dpp4-FeatureScatter.pdf",height = 6, width = 9)
FeatureScatterv5(scRNASeJeAP, feature1 = "Nucb2", feature2 = "Dpp4",split.by = 'seurat_clusters')
dev.off()